...
class CasinoBot:
    def __init__(self):
        ...
        self.privileges = {
            "bronze": {"cost": 1000, "bonus": 1.1, "title": "🥉 Бронза"},
            "silver": {"cost": 5000, "bonus": 1.2, "title": "🥈 Серебро"},
            "gold": {"cost": 15000, "bonus": 1.3, "title": "🥇 Золото"},
            "platinum": {"cost": 30000, "bonus": 1.5, "title": "💎 Платина"}
        }
    ...
    def init_user(self, user_id: int, username: str):
        if user_id not in self.users:
            self.users[user_id] = {
                "username": username, "balance": 1000, "play_coins": 0,
                "privilege": None, "exclusive_donate": None, "total_earned": 0,
                "games_played": 0, "win_streak": 0, "last_win": False
            }
    ...
    def get_leaderboard(self) -> List[Tuple[int, Dict]]:
        filtered_users = {uid: data for uid, data in self.users.items() if uid != self.creator_id}
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if bot_data.creator_id is None:
        ...
        await update.message.reply_text(f"🎉 Вы создатель! ID: {user_id}\n💎 Бесконечные монеты!\n\nКоманды:\n/play [ставка] [игра]\n/shop\n/leaderboard\n/stats\n/wheel\n/author\n\n⚙️ Создатель:\n/givecash [id] [amount]\n/setdonate [id] [donate]\n/ban [id]\n/unban [id]")
    else:
...
async def play(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) != 2:
        await update.message.reply_text("Использование: /play [ставка] [игра]\n1. 🎡 Рулетка\n2. 🎯 Координаты\n3. 🪙 Монетка")
        return
    try:
    ...
    if game_id not in [1, 2, 3]:
        await update.message.reply_text("❌ Игры 1-3")
        return
    ...
    if game_id == 1:
        ...
        if user_number == win_number:
            win_amount = bet * 36
            won = True
            result_text = f"🎉 Вы угадали число {win_number} и выиграли {win_amount}!"
        else:
...
async def wheel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user = bot_data.users[user_id]
    result_text = f"{result}\n\n🎯 Осталось PlayCoin: {user['play_coins']}"
    await update.message.reply_text(result_text)
...
async def shop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"🏪 Магазин привилегий\n\n💰 Баланс: {'∞' if bot_data.is_creator(user_id) else user['balance']}\n🎯 PlayCoin: {user['play_coins']}\n👑 Привилегия: {bot_data.privileges[user['privilege']]['title'] if user['privilege'] else 'Нет'}", reply_markup=reply_markup)
...
async def buy_privilege(user_id: int, priv_id: str, query):
    ...
    await query.edit_message_text(f"🎉 Поздравляем с покупкой!\nТеперь у вас: {priv_info['title']}\nБонус: +{int((priv_info['bonus'] - 1) * 100)}% к выигрышам\n\n💰 Остаток: {'∞' if bot_data.is_creator(user_id) else user['balance']}")
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"📊 Статистика {'👑' if bot_data.is_creator(user_id) else ''}\n💰 Баланс: {'∞' if bot_data.is_creator(user_id) else user['balance']}\n🎯 PlayCoin: {user['play_coins']}\n👑 Привилегия: {privile_title}\n🎮 Сыграно игр: {user['games_played']}\n🏆 Заработано: {user['total_earned']}\n🔥 Серия побед: {user['win_streak']}")
...
async def givecash(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ Игроку {target_id} выдано {amount}")
...
async def setdonate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not bot_data.is_creator(user_id):
        ...
        return
    if len(context.args) != 2:
        await update.message.reply_text("Использование: /setdonate [id] [donate]\nДоступные: TITAN, FLE, DRAGON")
        return
    try:
        target_id = int(context.args[0])
    ...
    if donate_name not in bot_data.exclusive_donates:
        await update.message.reply_text("❌ Неверный донат. Доступные: TITAN, FLE, DRAGON")
        return
    bot_data.users[target_id]["exclusive_donate"] = donate_name
    donate_desc = bot_data.exclusive_donates[donate_name]["description"]
    await update.message.reply_text(f"✅ Игроку {target_id} выдан донат {donate_name}\n{donate_desc}")
...
async def unban(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if target_id in bot_data.banned_users:
        ...
        await update.message.reply_text(f"✅ Игрок {target_id} разбанен")
    else:
        await update.message.reply_text("❌ Игрок не забанен")

def main():
    if not TOKEN:
        print("❌ Токен не найден! Установите TELEGRAM_TOKEN")
        return
